...
- [x] T106 [P] Verify responsiveness at 360 px for `HomePage` (mapa + popup), `StationPanel` (charts), `LoginPage`, `RegisterPage`, `FavoritesPage`, `AlertsPage`; fix any horizontal scroll or overlap in the relevant component files. **Audit**: layout ya mobile-first (sin anchos fijos > 360, grids colapsan a 1 col, paneles/forms full-width, header con breakpoints `sm:`). **Hardening aplicado** contra desbordes por nombres de estación largos (relevante cuando OpenAQ alimente nombres reales en feature 002): `min-w-0` + `break-words` en las filas flex con botón fijo de `StationPanel`, `StationPopup`, `FavoriteCard`, `AlertList`, `AlertHistory`; `truncate max-w-[180px]` en el email del `Header`. Build + 35 tests de los componentes tocados en verde.
- [x] T107 [P] Add `ReconnectingIndicator` component reflecting Realtime channel status (CONNECTING / DISCONNECTED) in `src/components/layout/ReconnectingIndicator.tsx`. Solo visible en `CONNECTING`/`DISCONNECTED` (spinner / `WifiOff`), `role="status"` + `aria-live`, texto oculto en móvil. 3 tests.
- [x] T108 [P] Wire `ReconnectingIndicator` into `Header` using channel status exposed by the realtime hooks in `src/components/layout/Header.tsx`. Puente vía `realtimeStore` (Zustand): `MapView` sincroniza el `status` de `useReadingsRealtime` al store y lo restaura a `CONNECTED` al desmontar (default neutro → indicador oculto en pantallas sin mapa). Store con su test; suite 166 tests en verde, coverage 97.25%/90.36%.
- [x] T109 [P] Manual test of the "no data yet" first-boot path: empty DB → confirm map shows all-gray markers + banner; documented in `specs/001-air-quality-dashboard/quickstart.md`. Documentado además el escenario de **datos rancios** (marcador con color vía `latest_station_readings` mientras las tendencias salen vacías por el filtro de rango) con su método de verificación (`max(measured_at)`).
...
- [x] T111 [P] Deploy to Vercel with `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` configured in project env vars (one-off ops step). **Prep listo**: `vercel.json` en la raíz con rewrite SPA → `/index.html`; `npm run build` pasa limpio (output en `dist/`, 1021 kB / 291 kB gzip); `.env.local` confirmado en `.gitignore` y sin trackear; auditoría del bundle confirma que la `service_role` NO aparece y la única key embebida es la `anon` (`role:"anon"` decodificada del JWT). **Desplegado en vivo (2026-05-30)**: https://air-vision-xi.vercel.app/ — público (verificado en incógnito), env vars configuradas en Vercel.
- [x] T112 [P] Run a coverage report (`npm run test:coverage`) and confirm thresholds pass on `src/hooks/`, `src/utils/`, `src/stores/`. Fix any shortfalls by adding missing tests in the corresponding `.test.ts(x)` files. **Resultado** (166 tests, exit 0): All files 97.25% líneas / 90.36% branches; por directorio — hooks 99.35/90, lib 100/100, stores 93.70/87.73, utils 100/100. Umbrales config (`vitest.config.ts`): lines/functions/statements 70, branches 65 → pasan holgadamente. Reporte HTML en `coverage/index.html` (gitignored).
...
- [x] T114 [P] Capture 3 screenshots (`docs/screenshots/map.png`, `dashboard.png`, `alerts.png`) for the README. Capturadas manualmente (~1920×945) y cableadas en la sección Demo del README vía tabla HTML. De paso: conteo de tests 161→166 y coverage 97.25%/90.36% actualizados en README.
- [x] T115 Run the full Quality Gates checklist from constitution (build / typecheck / lint / tests + coverage / security review / architectural review / UX review) and resolve any failures. **7/7 PASS**, documentado en `specs/001-air-quality-dashboard/checklists/quality-gates.md`. Failure resuelto: gate 3 (Prettier marcaba 58 archivos, causa raíz CRLF/LF en Windows) → `"endOfLine": "auto"` en `.prettierrc` + tooling vendado de Spec Kit (`.specify/`, `.claude/`) añadido a `.prettierignore`. Build/typecheck/lint/prettier exit 0; 166 tests, coverage 97.25%/90.36%; RLS en las 5 tablas; sin `service_role` en cliente; sin imports de Supabase en componentes ni llamadas externas en `src`.
- [~] T116 [omitido] Final pass: open the deployed Vercel URL, walk through US1→US5 as a user (no DevTools), confirm Spanish copy is correct and tones are consistent, record a 60-second screencast for the portfolio. **Omitido por decisión del owner** (2026-05-30): era el paso opcional "nice-to-have" del plan; el portafolio queda demostrable con README + screenshots (T113/T114). Se puede retomar cuando haya una URL desplegada estable.
