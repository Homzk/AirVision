...
- [ ] T111 [P] Deploy to Vercel with `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` configured in project env vars (one-off ops step). **Prep listo**: `vercel.json` en la raíz con rewrite SPA → `/index.html`; `npm run build` pasa limpio (output en `dist/`, 1021 kB / 291 kB gzip); `.env.local` confirmado en `.gitignore` y sin trackear; auditoría del bundle confirma que la `service_role` NO aparece y la única key embebida es la `anon` (`role:"anon"` decodificada del JWT). Falta solo el `vercel deploy` + setear env vars en el dashboard.
